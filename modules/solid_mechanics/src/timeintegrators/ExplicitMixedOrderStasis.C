...
ExplicitMixedOrderStasis::validParams()
{
  ...
  params.addClassDescription(
			     "ExplicitMixedOrder time integrator, with the additional ability to perform time steps in which velocity and acceleration are set to zero, and the primary variables are not changed");

  params.addRequiredParam<PostprocessorName>("in_stasis", "Postprocessor that defines whether the time stepping is in stasis or not.  If in_stasis==1 then time stepping involves setting v=0, and a=0 for second-order variables, and not changing the primary variables.  Otherwise, ordinary time stepping is used, as per the ExplicitMixedOrder time stepper");
  return params;
}
