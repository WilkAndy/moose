...
const PertinentGeochemicalSystem model_good_calcite(
    database_good_calcite, {"H2O", "H+", "HCO3-", "Ca++"}, {"Calcite"}, {}, {}, {}, {});
...
const EquilibriumGeochemicalSystem egs_good_calcite(model_good_calcite.modelGeochemicalDatabase(),
                                                    1E-6,
                                                    {"Ca++"},
                                                    {"Calcite"},
                                                    "H+",
...
TEST(EquilibiumGeochemicalSystemTest, constructWithMultiMooseEnum)
{
  ...
  const EquilibriumGeochemicalSystem egs(model_good_calcite.modelGeochemicalDatabase(),
                                         1E-6,
                                         {"Ca++"},
                                         {"Calcite"},
                                         "H+",
...
TEST(EquilibiumGeochemicalSystemTest, kineticException)
{
  ...
  try
  {
    EquilibriumGeochemicalSystem egs(
        model.modelGeochemicalDatabase(), 1E-6, {}, {}, "H+", {}, {}, dummy, 25, 3.0, 0, 1E-20);
...
TEST(EquilibiumGeochemicalSystemTest, swapSizeException)
{
  ...
  {
    EquilibriumGeochemicalSystem egs(
        model.modelGeochemicalDatabase(), 1E-6, {"H+"}, {}, "H+", {}, {}, dummy, 25, 3.0, 0, 1E-20);
    FAIL() << "Missing expected exception.";
  }
  catch (const std::exception & e)
  {
...
{
  GeochemicalDatabaseReader database("database/moose_testdb.json");
  PertinentGeochemicalSystem model(database, {"H2O", "H+", "HCO3-"}, {}, {}, {}, {}, {});
...
TEST(EquilibiumGeochemicalSystemTest, positiveException3)
{
  ...
  PertinentGeochemicalSystem model(
      database, {"H2O", "H+", "HCO3-", "O2(aq)"}, {}, {"O2(g)"}, {}, {}, {});
...
TEST(EquilibiumGeochemicalSystemTest, positiveException5)
{
  GeochemicalDatabaseReader database("database/moose_testdb.json");
  PertinentGeochemicalSystem model(
      database, {"H2O", "H+", "HCO3-", "Ca++"}, {"Calcite"}, {}, {}, {}, {});
...
TEST(EquilibiumGeochemicalSystemTest, mineralMeaningExecption)
{
  ...
  try
  {
    EquilibriumGeochemicalSystem egs(model_good_calcite.modelGeochemicalDatabase(),
                                     1E-6,
                                     {"Ca++"},
                                     {"Calcite"},
                                     "H+",
...
/// Check getDebyeHuckel
TEST(EquilibiumGeochemicalSystemTest, getDebyeHuckel)
{
  const std::vector<Real> dh_gold = {0.5092,
                                     ...
                                     0.001545,
                                     0.0};
  EXPECT_EQ(egs_good_calcite.getDebyeHuckel().size(), dh_gold.size());
  for (unsigned i = 0; i < dh_gold.size(); ++i)
    EXPECT_EQ(egs_good_calcite.getDebyeHuckel()[i], dh_gold[i]);
}
...
TEST(EquilibiumGeochemicalSystemTest, getBasisActivity)
{
  ...
  EquilibriumGeochemicalSystem egs(
      model.modelGeochemicalDatabase(),
      1E-6,
      {"O2(aq)"},
      {"O2(g)"},
      "H+",
...
TEST(EquilibiumGeochemicalSystemTest, getTotalCharge)
{
  ...
  const PertinentGeochemicalSystem model(
      database, {"H2O", "StoiCheckBasis", "Fe+++"}, {}, {}, {}, {}, {});
  const std::vector<EquilibriumGeochemicalSystem::ConstraintMeaningEnum> cm = {
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::KG_SOLVENT_WATER,
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::MOLES_BULK_SPECIES,
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::MOLES_BULK_SPECIES};
  const EquilibriumGeochemicalSystem egs(model.modelGeochemicalDatabase(),
                                         1E-6,
                                         {},
                                         {},
                                         "StoiCheckBasis",
...
TEST(EquilibiumGeochemicalSystemTest, getResidual)
{
  ...
  Real res = -175.0 + nw * MOLES_PER_KG_WATER;
  for (unsigned j = 0; j < mgd.eqm_species_name.size(); ++j)
    res += nw * mgd.eqm_stoichiometry(j, 0) * egs.getEquilibriumMolality(j);
  ...
  for (unsigned j = 0; j < mgd.eqm_species_name.size(); ++j)
    res += nw * mgd.eqm_stoichiometry(j, 2) * egs.getEquilibriumMolality(j);
  ...
  PertinentGeochemicalSystem model2(
      database_good_calcite, {"H2O", "H+", "StoiCheckBasis"}, {}, {}, {}, {}, {});
  const std::vector<EquilibriumGeochemicalSystem::ConstraintMeaningEnum> cm2 = {
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::MOLES_BULK_WATER,
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::ACTIVITY,
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::MOLES_BULK_SPECIES};
  const EquilibriumGeochemicalSystem egs2(model2.modelGeochemicalDatabase(),
                                          1E-6,
                                          {},
                                          {},
                                          "StoiCheckBasis",
  ...
  // H2O
  res = -175.0 + nw2 * (MOLES_PER_KG_WATER + (-0.5) * mscr);
 ...
 * Note that because the Jacobian calculations do not consider derivatives of activity or activity
 * coefficients, they are only roughly correct
 */
TEST(EquilibiumGeochemicalSystemTest, jac1)
{
  const std::vector<EquilibriumGeochemicalSystem::ConstraintMeaningEnum> cm = {
      ...
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::FREE_MOLALITY};
  EquilibriumGeochemicalSystem egs(
      model_good_calcite.modelGeochemicalDatabase(),
      1E-6,
      {"Ca++"},
      {"Calcite"},
      "HCO3-",
      ...
      cm,
      25,
      1E-8, // so activity coefficients are ~1
      4,
      1E-20);
  // Here, water and HCO3- are the components in the algebraic system
  EXPECT_EQ(egs.getNumInAlgebraicSystem(), 2);
...
TEST(EquilibiumGeochemicalSystemTest, jac3)
{
  ...
  EquilibriumGeochemicalSystem egs(model_good_calcite.modelGeochemicalDatabase(),
                                   ...
                                   cm,
                                   25,
                                   1E-8, // so activity coefficients ~1
                                   0,
                                   1E-20);
...
TEST(EquilibiumGeochemicalSystemTest, swapExceptions)
{
  ...
  cm2.push_back(EquilibriumGeochemicalSystem::ConstraintMeaningEnum::FUGACITY);
  cm2.push_back(EquilibriumGeochemicalSystem::ConstraintMeaningEnum::FREE_MOLALITY);
  EquilibriumGeochemicalSystem egs2(model.modelGeochemicalDatabase(),
                                    1E-6,
                                    {"O2(aq)"},
                                    {"O2(g)"},
                                    "H+",
...
/// check get and set max ionic strength
TEST(EquilibiumGeochemicalSystemTest, getsetmaxIS)
{
  EquilibriumGeochemicalSystem nonconst = egs_good_calcite;
  nonconst.setMaxIonicStrength(0.125);
  EXPECT_EQ(nonconst.getMaxIonicStrength(), 0.125);
}
...
TEST(EquilibiumGeochemicalSystemTest, alterAndRevertChargeBalance)
{
  const GeochemicalDatabaseReader database("database/moose_testdb.json");
  const PertinentGeochemicalSystem model(
      database, {"H2O", "StoiCheckBasis", "HCO3-"}, {}, {}, {}, {}, {});
  const std::vector<EquilibriumGeochemicalSystem::ConstraintMeaningEnum> cm = {
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::KG_SOLVENT_WATER,
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::MOLES_BULK_SPECIES,
      EquilibriumGeochemicalSystem::ConstraintMeaningEnum::MOLES_BULK_SPECIES};
