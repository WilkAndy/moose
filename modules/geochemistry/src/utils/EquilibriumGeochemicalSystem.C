...
EquilibriumGeochemicalSystem::EquilibriumGeochemicalSystem(
    ...
    Real approximate_initial_temperature,
...
EquilibriumGeochemicalSystem::checkAndInitialize()
{
  ...
  // order the constraints in the same way as the basis species.  This makes the following code much
  // cleaner.
...
EquilibriumGeochemicalSystem::computeConsistentConfiguration()
{
  ...
  // conventional approach: there are just too many unknowns and approximations to justify iterating
  // towards the perfectly consistent initial condition
  for (unsigned picard = 0; picard < _iters_to_make_consistent + 1; ++picard)
  {
    ...
    computeIonicStrengths(_ionic_strength, _stoichiometric_ionic_strength);
    buildActivityCoefficients(_basis_activity_coef, _eqm_activity_coef);
...
std::vector<Real>
EquilibriumGeochemicalSystem::getDebyeHuckel() const
{
    ...
    switch (meaning)
    {
      case ConstraintMeaningEnum::MOLES_BULK_WATER:
      {
        ...
        basis_molality[i] = std::max(
            ...
            0.999 * value /
                moles_per_kg_water); // mass of solvent water (water is an algebraic variable).
                                     // Guess used to initialize the Newton process
        break;
      }
      case ConstraintMeaningEnum::KG_SOLVENT_WATER:
      {
        bulk_moles[i] = value * moles_per_kg_water /
...
void
EquilibriumGeochemicalSystem::computeIonicStrengths(Real & ionic_strength,
                                                    Real & stoichiometric_ionic_strength) const
{
  ionic_strength = std::max(
      0.0,
      std::min(_max_ionic_strength,
    ...
    std::vector<Real> & basis_activity) const
{
  if (!_basis_activity_known[0])
    basis_activity[0] = std::exp(GeochemistryActivity::lnActivityDHBdotWater(
...
void
EquilibriumGeochemicalSystem::buildActivityCoefficients(std::vector<Real> & basis_activity_coef,
                                                        std::vector<Real> & eqm_activity_coef) const
{
  basis_activity_coef.resize(_num_basis);
  eqm_activity_coef.resize(_num_eqm);
  const Real sqrt_ionic_str = std::sqrt(_ionic_strength);

  for (unsigned basis_ind = 1; basis_ind < _num_basis; ++basis_ind) // don't loop over water
    if (_mgd.basis_species_mineral[basis_ind] || _mgd.basis_species_gas[basis_ind])
      continue; // these should never be used
    else if (_mgd.basis_species_radius[basis_ind] == -0.5)
    {
      basis_activity_coef[basis_ind] = std::pow(10.0,
                                                GeochemistryActivity::log10ActCoeffDHBdotNeutral(
                                                    _ionic_strength, _dha, _dhb, _dhc, _dhd));
    }
    else if (_mgd.basis_species_radius[basis_ind] == -1.0)
    {
      basis_activity_coef[basis_ind] = std::pow(
          10.0, GeochemistryActivity::log10ActCoeffDHBdotAlternative(_ionic_strength, _dhBdot));
    }
    else if (_mgd.basis_species_charge[basis_ind] == 0.0)
    {
      basis_activity_coef[basis_ind] = 1.0;
    }
    else
    {
      basis_activity_coef[basis_ind] =
          std::pow(10.0,
                   GeochemistryActivity::log10ActCoeffDHBdot(_mgd.basis_species_charge[basis_ind],
                                                             _mgd.basis_species_radius[basis_ind],
                                                             sqrt_ionic_str,
                                                             _dhA,
                                                             _dhB,
                                                             _dhBdot));
  ...
  for (unsigned eqm_i = 0; eqm_i < _num_eqm; ++eqm_i)
    if (_mgd.eqm_species_mineral[eqm_i] || _mgd.eqm_species_gas[eqm_i])
      continue;
    else if (_mgd.eqm_species_radius[eqm_i] == -0.5)
    {
      eqm_activity_coef[eqm_i] = std::pow(10.0,
                                          GeochemistryActivity::log10ActCoeffDHBdotNeutral(
                                              _ionic_strength, _dha, _dhb, _dhc, _dhd));
    }
    else if (_mgd.eqm_species_radius[eqm_i] == -1.0)
    {
      eqm_activity_coef[eqm_i] = std::pow(
          10.0, GeochemistryActivity::log10ActCoeffDHBdotAlternative(_ionic_strength, _dhBdot));
    }
    else if (_mgd.eqm_species_charge[eqm_i] == 0.0)
    {
      eqm_activity_coef[eqm_i] = 1.0;
    }
    else
    {
      eqm_activity_coef[eqm_i] =
          std::pow(10.0,
...
EquilibriumGeochemicalSystem::computeBulk(std::vector<Real> & bulk_moles) const
{
  ...
  for (unsigned i = 0; i < _num_basis; ++i)
  {
    ...
    switch (meaning)
    {
      ...
      case ConstraintMeaningEnum::KG_SOLVENT_WATER:
      {
        bulk_moles[i] = moles_per_kg_water;
...
EquilibriumGeochemicalSystem::getResidualComponent(unsigned algebraic_ind) const
{
  ...
  Real res = 0.0;
  if (basis_i == 0)
    res += -_bulk_moles[basis_i] + _basis_molality[0] * moles_per_kg_water;
...
void
EquilibriumGeochemicalSystem::setMaxIonicStrength(Real max_is)
{
  _max_ionic_strength = max_is;
}

Real
EquilibriumGeochemicalSystem::getMaxIonicStrength() const
{
