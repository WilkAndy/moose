...
public:
   ...
   * @param stoichiometry_tolerance Stoichiometric coefficients of magnitude less than this will be
  set to zero
   ...
   * @param max_ionic_strength Maximum value of the ionic strength used in Debye-Huckel computations
   ...
   * @param ionic_str_using_basis_only  If true, use only the basis molalities in the ionic strengh
  and stoichiometric ionic strength calculations (for Debye-Huckel activities).  Since basis
  molality is usually much greater than equilibrium molality, and the whole concept of activity
  coefficients depending on ionic strength is only approximate in practice, setting
  use_only_basis_molality=true is often a reasonable approximation, and it can help with convergence
  of Newton process that finds the solution of the equilibrium geochemical system.
   */
  EquilibriumGeochemicalSystem(const ModelGeochemicalDatabase & mgd,
  ...
  /**
   * Set the value of maximum ionic strength.  Note: this does not recompute ionic strengths,
   * activity coefficients, etc.  If you want to recompute these things based on your new maximum
   * ionic strength, use setAlgebraicVariables(getAlgebraicVariableDenseValues());
   */
  void setMaxIonicStrength(Real max_is);

  /// Get the value of maximum ionic strength
